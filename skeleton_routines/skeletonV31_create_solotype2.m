%
% this function creates a skeleton V3.1 b-Argo profile file with N_PROF=7
%
...
%
% THIS TEMPLATE WAS GENERATED IN SUPPORT OF THE FIRST MRV BSOLO TEST FLOAT IN THE GOBGC PROGRAM.
...
% Updated 7/2/24 to include CHLA_FLUORESCENCE.  TM.
%--------------------------------------------------------------

% filenameBR='TanyaTestBR2.nc';
% nlevels=192;
% nvalues=48; %dimension of uv spectra.
% TM; 06/7/22; these are now defined in MBARImat_to_ARGOb
%nprof = 7;
%nparam = 9;
ncid=netcdf.create(filenameBR,'clobber');

BPARAMS = {'DOWN_IRRADIANCE380',...
    'DOWN_IRRADIANCE443',...
    'DOWN_IRRADIANCE490',...
    'DOWN_IRRADIANCE555'};
IPARAMS = {'RAW_DOWNWELLING_IRRADIANCE380',...
    'RAW_DOWNWELLING_IRRADIANCE443',...
    'RAW_DOWNWELLING_IRRADIANCE490',...
    'RAW_DOWNWELLING_IRRADIANCE555'};
IPARAMlongname = {'Raw downwelling irradiance at 380 nanometers',...
...
BPARAMlongname = {'Downwelling irradiance at 380 nanometers',...
    'Downwelling irradiance at 443 nanometers',...
    'Downwelling irradiance at 490 nanometers',...
    'Downwelling irradiance at 555 nanometers'};
BPARAMstdname = {'',...
    '',...
    '',...
    ''};
BPARAMunits = {'W/m^2/nm',...
    'W/m^2/nm',...
    'W/m^2/nm',...
    'W/m^2/nm'};
BPARAMres = {'single(1e-6)',...
    'single(1e-6)',...
    'single(1e-6)',...
    'single(1e-6)'};
BPARAMcformat = {'%.6f',...
...
nprof_dimid=netcdf.defDim(ncid,'N_PROF',nprof);
...
netcdf.putAtt(ncid,varid,'units','days since 1950-01-01 00:00:00 UTC');
netcdf.putAtt(ncid,varid,'conventions','Relative julian days with decimal part (as parts of day)');
netcdf.putAtt(ncid,varid,'resolution',1./(24*60)); %TM from JG: the SOLO family returns JULD_LOCATION to the nearest minute.
netcdf.putAtt(ncid,varid,'_FillValue',999999.);
...
netcdf.putAtt(ncid,varid,'C_format','%7.2f'); %7.2 for SOLO (from JG)
netcdf.putAtt(ncid,varid,'FORTRAN_format','F7.2');
netcdf.putAtt(ncid,varid,'_FillValue',single(99999.));
netcdf.putAtt(ncid,varid,'axis','Z');

% OXYGEN SENSOR:--------------------------------------------------------------------

INDo = strfind(INFO.sensors,'DOX');

...
%
% APEX Iridium with Aanderaa 4330 (doxy), ISUS (nitrate), DURA (ph), FL2BB (chla, chla435 & bb)
%
% 6 B-Argo parameters: DOXY, CHLA, CHLA435, BBP700, PH_IN_SITU_TOTAL, NITRATE
%
% 12 I-Argo parameters: TEMP_DOXY, TPHASE_DOXY, FLUORESCENCE_CHLA, FLUORESCENCE_CHLA435, BETA_BACKSCATTERING700, VRS_PH, IB_PH, IK_PH, VK_PH PH_IN_SITU_FREE, UV_INTENSITY_PARK_NITRATE, UV_INTENSITY_NITRATE (spectral)
%
% N_PARAM = 18 + 1 (PRES) = 19 (without chla435 should be 17)
%
% Tanya Maurer, May 2023

% SB 11/22/24 This function strips the FLBBFL chla435 to allow this Bfile
% to be sent to the main directory at the gdac. Type is still technically
% 17 but modified to fit Argo standards.
%--------------------------------------------------------------
keyboard
% declare FillValue ------
...
% junkfluorescence435=char('FLUORESCENCE_CHLA435',enough64');
% fluorescence43564=junkfluorescence435(1,:);

% junkchla435=char('CHLA435',enough64');
% chla43564=junkchla435(1,:);
...
station_params=...
[pres64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64;
pres64, tempdoxy64, tphasedoxy64, doxy64, fluorescence64, chla64, backscattering64, bbp64, vrsph64, ibph64, ikph64, vkph64, phfree64, phtotal64, uvdarknitrate64, uvnitrate64, nitrate64];
...
clear junk64 junkpres junktempdoxy junktphasedoxy junkdoxy junkfluorescence junkchla junkbackscattering junkbbp junkvrsph junkibph junkikph junkvkph junkphfree junkphtotal junkuvdarknitrate junkuvnitrate junknitrate
...
% chla435datamode=INFO.CHLA435_DATA_MODE;
% if(chla435datamode=='A')datamode_check(2)=2;end
% if(chla435datamode=='D')datamode_check(2)=3;end
...
param_datamode=...
['R', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ';
'R', 'R', 'R', doxydatamode, 'R', chladatamode, 'R', 'R', bbpdatamode, 'R', 'R', 'R', 'R', 'R', phdatamode, 'R', 'R', nidatamode];
...
% fluorescence_chla435=LR.FLUORESCENCE_CHLA435;
% ii=find(fluorescence_chla435>99998);
% fluorescence_chla435(ii)=fillfloat;
% varid=netcdf.inqVarID(fid,'FLUORESCENCE_CHLA435');
% netcdf.putVar(fid, varid, [0 1], [nlowres 1], fluorescence_chla435);

% fluo_chla435_qc=num2str(zeros(nlowres,1));
% fluo_chla435_qc(ii)='9';
% varid=netcdf.inqVarID(fid,'FLUORESCENCE_CHLA435_QC');
% netcdf.putVar(fid, varid, [0 1], [nlowres 1], fluo_chla435_qc);

% chla435=LR.CHLA435;
% ii=find(chla435>99998);
% chla435(ii)=fillfloat;
% varid=netcdf.inqVarID(fid,'CHLA435');
% netcdf.putVar(fid, varid, [0 1], [nlowres 1], chla435);

% jj=find(LR.CHLA435_QC==99);
% if(length(jj)==nlowres & length(ii)==nlowres)
%   disp(strcat('no CHLA435 in cast :', num2str(INFO.cast)));
%   chla435_qc=num2str(ones(nlowres,1).*9);
% else
%   LR.CHLA435_QC(ii)=9;
%   chla435_qc=num2str(LR.CHLA435_QC);
% end
% varid=netcdf.inqVarID(fid,'CHLA435_QC');
% netcdf.putVar(fid, varid, [0 1], [nlowres 1], chla435_qc);
...
% chla435_adj=LR.CHLA435_ADJUSTED;
% ii=find(chla435_adj>99998);

% jj=find(LR.CHLA435_ADJUSTED_QC==99);
% if(length(jj)==nlowres & length(ii)==nlowres)
%   disp(strcat('no CHLA435_ADJUSTED in cast :', num2str(INFO.cast)));
% else
%   LR.CHLA435_ADJUSTED_QC(ii)=9;
%   chla435_adj_qc=num2str(LR.CHLA435_ADJUSTED_QC);
%   varid=netcdf.inqVarID(fid,'CHLA435_ADJUSTED_QC');
%   netcdf.putVar(fid, varid, [0 1], [nlowres 1], chla435_adj_qc);
% 
%   kk=find(chla435_adj_qc=='4');
%   chla435_adj(ii)=fillfloat; % qc='9'
%   if(chla435datamode=='D')
%     chla435_adj(kk)=fillfloat; % qc='4'
%   end
%   varid=netcdf.inqVarID(fid,'CHLA435_ADJUSTED');
%   netcdf.putVar(fid, varid, [0 1], [nlowres 1], chla435_adj);
% 
%   chla435_adj_error=LR.CHLA435_ADJUSTED_ERROR;
%   chla435_adj_error(ii)=fillfloat; % qc='9'
%   if(chla435datamode=='D')
%     chla435_adj_error(kk)=fillfloat; % qc='4'
%   end
%   varid=netcdf.inqVarID(fid,'CHLA435_ADJUSTED_ERROR');
%   netcdf.putVar(fid, varid, [0 1], [nlowres 1], chla435_adj_error);
% end
...
% junk=char(INFO.CHLA435_SCI_CAL_EQU,enough256');
% chla435_eqn256=junk(1,:);
% junk=char(INFO.CHLA435_SCI_CAL_COEF,enough256');
% chla435_coeff256=junk(1,:);
% junk=char(INFO.CHLA435_SCI_CAL_COM,enough256');
% chla435_comment256=junk(1,:);
...
% netcdf.putVar(fid, varid, [0,8-1,0,1], [256,1,1,1], chla435_comment256); %chla=8
netcdf.putVar(fid, varid, [0,10-1,0,1], [256,1,1,1], bbp700_comment256); %bbp700=10
netcdf.putVar(fid, varid, [0,16-1,0,1], [256,1,1,1], ph_comment256); %ph=16
netcdf.putVar(fid, varid, [0,19-1,0,1], [256,1,1,1], ni_comment256); %nitrate=19
...
% netcdf.putVar(fid, varid, [0,8-1,0,1], [256,1,1,1], chla435_eqn256); %chla=8
netcdf.putVar(fid, varid, [0,10-1,0,1], [256,1,1,1], bbp700_eqn256); %bbp700=10
netcdf.putVar(fid, varid, [0,16-1,0,1], [256,1,1,1], ph_eqn256); %ph=16
netcdf.putVar(fid, varid, [0,19-1,0,1], [256,1,1,1], ni_eqn256); %nitrate=19
...
% netcdf.putVar(fid, varid, [0,8-1,0,1], [256,1,1,1], chla435_coeff256); %chla=8
netcdf.putVar(fid, varid, [0,10-1,0,1], [256,1,1,1], bbp700_coeff256); %bbp700=10
netcdf.putVar(fid, varid, [0,16-1,0,1], [256,1,1,1], ph_coeff256); %ph=16
netcdf.putVar(fid, varid, [0,16-1,0,1], [256,1,1,1], ni_coeff256); %nitrate=19

...
ECOs = [fluorescencechla64, chla64, backscattering64, bbp64, fluorescencecdom64, cdom64];
...
clear TMP myrow station_params junk64 junkpres junktempdoxy junktphasedoxy junkdoxy junkfluorescence junkchla junkbackscattering junkbbp junkvrsph junkibph junkikph junkvkph junkphfree junkphtotal junkuvdarknitrate junkuvnitrate junknitrate junktemp_rawdwirr380 junktemp_rawdwirr412 junktemp_rawdwirr490 junktemp_rawdwpar junkdwirr380 junkdwirr412 junkdwirr490 junkdwpar
...
ECOsDM = ['R', 'R', chladatamode, 'R', bbpdatamode, 'R', cdomdatamode];
...
if FULLinds(1)>0 %doxy data exists
    temp_doxy=MYBGC{FULLinds(1)-2}.TEMP_DOXY;
    ii=find(temp_doxy>99998);
    temp_doxy(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'TEMP_DOXY');
    netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], temp_doxy);

    temp_doxy_qc=num2str(zeros(nbgcres(dox_ax-1,1),1));
    temp_doxy_qc(ii)='9';
    varid=netcdf.inqVarID(fid,'TEMP_DOXY_QC');
    netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], temp_doxy_qc);

    phase_delay_doxy=MYBGC{FULLinds(1)-2}.PHASE_DELAY_DOXY;
    ii=find(phase_delay_doxy>99998);
    phase_delay_doxy(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'PHASE_DELAY_DOXY');
    netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], phase_delay_doxy);

    phase_delay_doxy_qc=num2str(zeros(nbgcres(dox_ax-1,1),1));
    phase_delay_doxy_qc(ii)='9';
    varid=netcdf.inqVarID(fid,'PHASE_DELAY_DOXY_QC');
    netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], phase_delay_doxy_qc);

    doxy=MYBGC{FULLinds(1)-2}.DOXY;
    ii=find(doxy>99998);
    doxy(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'DOXY');
    netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], doxy);

    jj=find(MYBGC{FULLinds(1)-2}.DOXY_QC==99);
    if(length(jj)==nbgcres(dox_ax-1,1) & length(ii)==nbgcres(dox_ax-1,1))
        disp(strcat('no DOXY in cast :', num2str(INFO.cast)));
        doxy_qc=num2str(ones(nbgcres(dox_ax-1,1),1).*9);
    else
        MYBGC{FULLinds(1)-2}.DOXY_QC(ii)=9;
        doxy_qc=num2str(MYBGC{FULLinds(1)-2}.DOXY_QC);
    end
    varid=netcdf.inqVarID(fid,'DOXY_QC');
    netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], doxy_qc);

    %doxy adjusted fields
    doxy_adj=MYBGC{FULLinds(1)-2}.DOXY_ADJUSTED;
    ii=find(doxy_adj>99998);

    jj=find(MYBGC{FULLinds(1)-2}.DOXY_ADJUSTED_QC==99);
    if(length(jj)==nbgcres(dox_ax-1,1) & length(ii)==nbgcres(dox_ax-1,1))
        disp(strcat('no DOXY_ADJUSTED in cast :', num2str(INFO.cast)));
    else
        MYBGC{FULLinds(1)-2}.DOXY_ADJUSTED_QC(ii)=9;
        doxy_adj_qc=num2str(MYBGC{FULLinds(1)-2}.DOXY_ADJUSTED_QC);
        %varid=netcdf.inqVarID(fid,'DOXY_ADJUSTED_QC');
        %netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], doxy_adj_qc);
        ...
        netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], doxy_adj);
        varid=netcdf.inqVarID(fid,'DOXY_ADJUSTED_QC');
        netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], doxy_adj_qc);

        doxy_adj_error=MYBGC{FULLinds(1)-2}.DOXY_ADJUSTED_ERROR;
        doxy_adj_error(ii)=fillfloat; % qc='9'
        if(doxydatamode=='D')
            doxy_adj_error(kk)=fillfloat; % qc='4'
        end
        varid=netcdf.inqVarID(fid,'DOXY_ADJUSTED_ERROR');
        netcdf.putVar(fid, varid, [0 dox_ax], [nbgcres(dox_ax-1,1) 1], doxy_adj_error);
    end
end
...
if FULLinds(3)>0 %eco data exists
    ...
    fluorescence_chla=MYBGC{FULLinds(3)-2}.FLUORESCENCE_CHLA;
    ii=find(fluorescence_chla>99998);
    fluorescence_chla(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'FLUORESCENCE_CHLA');
    nbgcres
    eco_ax
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], fluorescence_chla);

    fluo_chla_qc=num2str(zeros(nbgcres(eco_ax-1,1),1));
    fluo_chla_qc(ii)='9';
    varid=netcdf.inqVarID(fid,'FLUORESCENCE_CHLA_QC');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], fluo_chla_qc);

    chla=MYBGC{FULLinds(3)-2}.CHLA;
    ii=find(chla>99998);
    chla(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'CHLA');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], chla);

    jj=find(MYBGC{FULLinds(3)-2}.CHLA_QC==99);
    if(length(jj)==nbgcres(eco_ax-1,1) & length(ii)==nbgcres(eco_ax-1,1))
        disp(strcat('no CHLA in cast :', num2str(INFO.cast)));
        chla_qc=num2str(ones(nbgcres(eco_ax-1,1),1).*9);
    else
        MYBGC{FULLinds(3)-2}.CHLA_QC(ii)=9;
        chla_qc=num2str(MYBGC{FULLinds(3)-2}.CHLA_QC);
    end
    varid=netcdf.inqVarID(fid,'CHLA_QC');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], chla_qc);

    beta_backscattering700=MYBGC{FULLinds(3)-2}.BETA_BACKSCATTERING700;
    ii=find(beta_backscattering700>99998);
    beta_backscattering700(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'BETA_BACKSCATTERING700');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], beta_backscattering700);

    beta_bbp700_qc=num2str(zeros(nbgcres(eco_ax-1,1),1));
    beta_bbp700_qc(ii)='9';
    varid=netcdf.inqVarID(fid,'BETA_BACKSCATTERING700_QC');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], beta_bbp700_qc);

    bbp700=MYBGC{FULLinds(3)-2}.BBP700;
    ii=find(bbp700>99998);
    bbp700(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'BBP700');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], bbp700);

    jj=find(MYBGC{FULLinds(3)-2}.BBP700_QC==99);
    if(length(jj)==nbgcres(eco_ax-1,1) & length(ii)==nbgcres(eco_ax-1,1))
        disp(strcat('no BBP700 in cast :', num2str(INFO.cast)));
        bbp700_qc=num2str(ones(nbgcres(eco_ax-1,1),1).*9);
    else
        MYBGC{FULLinds(3)-2}.BBP700_QC(ii)=9;
        bbp700_qc=num2str(MYBGC{FULLinds(3)-2}.BBP700_QC);
    end
    varid=netcdf.inqVarID(fid,'BBP700_QC');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], bbp700_qc);

    fluorescence_cdom=[MYBGC{FULLinds(3)-2}.FLUORESCENCE_CDOM];
    ii=find(fluorescence_cdom>99998);
    fluorescence_cdom(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'FLUORESCENCE_CDOM');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], fluorescence_cdom);

    fluo_cdom_qc=num2str(zeros(nbgcres(eco_ax-1,1),1));
    fluo_cdom_qc(ii)='9';
    varid=netcdf.inqVarID(fid,'FLUORESCENCE_CDOM_QC');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], fluo_cdom_qc);

    cdom=[MYBGC{FULLinds(3)-2}.CDOM];
    ii=find(cdom>99998);
    cdom(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'CDOM');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], cdom);

    ncdom_qc=[MYBGC{FULLinds(3)-2}.CDOM_QC];
    jj=find(ncdom_qc==99);
    if(length(jj)==nbgcres(eco_ax-1,1) & length(ii)==nbgcres(eco_ax-1,1))
        disp(strcat('no CDOM in cast :', num2str(INFO.cast)));
        cdom_qc=num2str(ones(nbgcres(eco_ax-1,1),1).*9);
    else
        ncdom_qc(ii)=9;
        cdom_qc=num2str(ncdom_qc);
    end
    varid=netcdf.inqVarID(fid,'CDOM_QC');
    netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], cdom_qc);

    chla_adj=MYBGC{FULLinds(3)-2}.CHLA_ADJUSTED;
    ii=find(chla_adj>99998);

    jj=find(MYBGC{FULLinds(3)-2}.CHLA_ADJUSTED_QC==99);
    if(length(jj)==nbgcres(eco_ax-1,1) & length(ii)==nbgcres(eco_ax-1,1))
        disp(strcat('no CHLA_ADJUSTED in cast :', num2str(INFO.cast)));
    else
        MYBGC{FULLinds(3)-2}.CHLA_ADJUSTED_QC(ii)=9;
        chla_adj_qc=num2str(MYBGC{FULLinds(3)-2}.CHLA_ADJUSTED_QC);
        %varid=netcdf.inqVarID(fid,'CHLA_ADJUSTED_QC');
        %netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], chla_adj_qc);
        ...
        netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], chla_adj);
        varid=netcdf.inqVarID(fid,'CHLA_ADJUSTED_QC');
        netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], chla_adj_qc);

        chla_adj_error=MYBGC{FULLinds(3)-2}.CHLA_ADJUSTED_ERROR;
        chla_adj_error(ii)=fillfloat; % qc='9'
        if(chladatamode=='D')
            chla_adj_error(kk)=fillfloat; % qc='4'
        end
        varid=netcdf.inqVarID(fid,'CHLA_ADJUSTED_ERROR');
        netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], chla_adj_error);
    end


    bbp700_adj=MYBGC{FULLinds(3)-2}.BBP700_ADJUSTED;
    ii=find(bbp700_adj>99998);

    jj=find(MYBGC{FULLinds(3)-2}.BBP700_ADJUSTED_QC==99);
    if(length(jj)==nbgcres(eco_ax-1,1) & length(ii)==nbgcres(eco_ax-1,1))
        disp(strcat('no BBP700_ADJUSTED in cast :', num2str(INFO.cast)));
    else
        MYBGC{FULLinds(3)-2}.BBP700_ADJUSTED_QC(ii)=9;
        bbp700_adj_qc=num2str(MYBGC{FULLinds(3)-2}.BBP700_ADJUSTED_QC);
        % varid=netcdf.inqVarID(fid,'BBP700_ADJUSTED_QC');
        % netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], bbp700_adj_qc);
        ...
        netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], bbp700_adj);
        varid=netcdf.inqVarID(fid,'BBP700_ADJUSTED_QC');
        netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], bbp700_adj_qc);

        bbp700_adj_error=MYBGC{FULLinds(3)-2}.BBP700_ADJUSTED_ERROR;
        bbp700_adj_error(ii)=fillfloat; % qc='9'
        if(bbpdatamode=='D')
            bbp700_adj_error(kk)=fillfloat; % qc='4'
        end
        varid=netcdf.inqVarID(fid,'BBP700_ADJUSTED_ERROR');
        netcdf.putVar(fid, varid, [0 eco_ax], [nbgcres(eco_ax-1,1) 1], bbp700_adj_error);
    end
end
...
if FULLinds(2)>0 %pH data exists
    ...
    vrs_ph=MYBGC{FULLinds(2)-2}.VRS_PH;
    ii=find(vrs_ph>99998);
    vrs_ph(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'VRS_PH');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], vrs_ph);

    vrs_ph_qc=num2str(zeros(nbgcres(ph_ax-1,1),1));
    vrs_ph_qc(ii)='9';
    varid=netcdf.inqVarID(fid,'VRS_PH_QC');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], vrs_ph_qc);

    ib_ph=MYBGC{FULLinds(2)-2}.IB_PH;
    ii=find(ib_ph>99998);
    ib_ph(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'IB_PH');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ib_ph);

    ib_ph_qc=num2str(zeros(nbgcres(ph_ax-1,1),1));
    ib_ph_qc(ii)=' ';
    varid=netcdf.inqVarID(fid,'IB_PH_QC');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ib_ph_qc);

    ik_ph=MYBGC{FULLinds(2)-2}.IK_PH;
    ii=find(ik_ph>99998);
    ik_ph(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'IK_PH');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ik_ph);

    ik_ph_qc=num2str(zeros(nbgcres(ph_ax-1,1),1));
    ik_ph_qc(ii)=' ';
    varid=netcdf.inqVarID(fid,'IK_PH_QC');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ik_ph_qc);

    vk_ph=MYBGC{FULLinds(2)-2}.VK_PH;
    ii=find(vk_ph>99998);
    vk_ph(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'VK_PH');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], vk_ph);

    vk_ph_qc=num2str(zeros(nbgcres(ph_ax-1,1),1));
    vk_ph_qc(ii)=' ';
    varid=netcdf.inqVarID(fid,'VK_PH_QC');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], vk_ph_qc);

    ph_in_situ_free=MYBGC{FULLinds(2)-2}.PH_IN_SITU_FREE;
    ii=find(ph_in_situ_free>99998);
    ph_in_situ_free(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'PH_IN_SITU_FREE');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ph_in_situ_free);

    ph_in_situ_free_qc=num2str(zeros(nbgcres(ph_ax-1,1),1));
    ph_in_situ_free_qc(ii)='9';
    varid=netcdf.inqVarID(fid,'PH_IN_SITU_FREE_QC');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ph_in_situ_free_qc);

    ph_in_situ_total=MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL;
    ii=find(ph_in_situ_total>99998);
    ph_in_situ_total(ii)=fillfloat;
    varid=netcdf.inqVarID(fid,'PH_IN_SITU_TOTAL');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ph_in_situ_total);

    jj=find(MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL_QC==99);
    if(length(jj)==nbgcres(ph_ax-1,1) & length(ii)==nbgcres(ph_ax-1,1))
        disp(strcat('no PH_IN_SITU_TOTAL in cast :', num2str(INFO.cast)));
        ph_in_situ_total_qc=num2str(ones(nbgcres(ph_ax-1,1),1).*9);
    else
        MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL_QC(ii)=9;
        ph_in_situ_total_qc=num2str(MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL_QC);
    end
    varid=netcdf.inqVarID(fid,'PH_IN_SITU_TOTAL_QC');
    netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ph_in_situ_total_qc);

    ph_in_situ_total_adj=MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL_ADJUSTED;
    ii=find(ph_in_situ_total_adj>99998);

    jj=find(MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL_ADJUSTED_QC==99);
    if(length(jj)==nbgcres(ph_ax-1,1) & length(ii)==nbgcres(ph_ax-1,1))
        disp(strcat('no PH_IN_SITU_TOTAL_ADJUSTED in cast :', num2str(INFO.cast)));
    else
        MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL_ADJUSTED_QC(ii)=9;
        ph_in_situ_total_adj_qc=num2str(MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL_ADJUSTED_QC);
        %varid=netcdf.inqVarID(fid,'PH_IN_SITU_TOTAL_ADJUSTED_QC');
        %netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ph_in_situ_total_adj_qc);
        ...
        netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ph_in_situ_total_adj);
        varid=netcdf.inqVarID(fid,'PH_IN_SITU_TOTAL_ADJUSTED_QC');
        netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ph_in_situ_total_adj_qc);

        ph_in_situ_total_adj_error=MYBGC{FULLinds(2)-2}.PH_IN_SITU_TOTAL_ADJUSTED_ERROR;
        ...
        netcdf.putVar(fid, varid, [0 ph_ax], [nbgcres(ph_ax-1,1) 1], ph_in_situ_total_adj_error);
    end
end
...
if FULLinds(5)>0 %no3 data exists
    ...
    uv_intensity_dark_nitrate=MYBGC{FULLinds(5)-2}.UV_INTENSITY_DARK_NITRATE;
    if ~isempty(uv_intensity_dark_nitrate)
        ...
        netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], uv_intensity_dark_nitrate);

        uv_intensity_dark_nitrate_qc=num2str(zeros(nbgcres(nit_ax-1,1),1));
        uv_intensity_dark_nitrate_qc(ii)='9';
        varid=netcdf.inqVarID(fid,'UV_INTENSITY_DARK_NITRATE_QC');
        netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], uv_intensity_dark_nitrate_qc);

        uv_intensity_nitrate=MYBGC{FULLinds(5)-2}.UV_INTENSITY_NITRATE;
        varid=netcdf.inqVarID(fid,'UV_INTENSITY_NITRATE');
        if(nvalues>1)
            ii=[];
            for j=1:nbgcres(nit_ax-1,1)
        ...
        elseif(nvalues==1)
            ...
            netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], uv_intensity_nitrate);
        end

        uv_intensity_nitrate_qc=num2str(zeros(nbgcres(nit_ax-1,1),1));
        uv_intensity_nitrate_qc(ii)='9';
        varid=netcdf.inqVarID(fid,'UV_INTENSITY_NITRATE_QC');
        netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], uv_intensity_nitrate_qc);

        nitrate=MYBGC{FULLinds(5)-2}.NITRATE;
        ii=find(nitrate>99998);
        nitrate(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'NITRATE');
        netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], nitrate);

        jj=find(MYBGC{FULLinds(5)-2}.NITRATE_QC==99);
        if(length(jj)==nbgcres(nit_ax-1,1) & length(ii)==nbgcres(nit_ax-1,1))
            disp(strcat('no NITRATE in cast :', num2str(INFO.cast)));
            nitrate_qc=num2str(ones(nbgcres(nit_ax-1,1),1).*9);
        else
            MYBGC{FULLinds(5)-2}.NITRATE_QC(ii)=9;
            nitrate_qc=num2str(MYBGC{FULLinds(5)-2}.NITRATE_QC);
        end
        varid=netcdf.inqVarID(fid,'NITRATE_QC');
        netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], nitrate_qc);

        nitrate_adj=MYBGC{FULLinds(5)-2}.NITRATE_ADJUSTED;
        ii=find(nitrate_adj>99998);

        jj=find(MYBGC{FULLinds(5)-2}.NITRATE_ADJUSTED_QC==99);
        if(length(jj)==nbgcres(nit_ax-1,1) & length(ii)==nbgcres(nit_ax-1,1))
            disp(strcat('no NITRATE_ADJUSTED in cast :', num2str(INFO.cast)));
        else
            MYBGC{FULLinds(5)-2}.NITRATE_ADJUSTED_QC(ii)=9;
            nitrate_adj_qc=num2str(MYBGC{FULLinds(5)-2}.NITRATE_ADJUSTED_QC);
            %varid=netcdf.inqVarID(fid,'NITRATE_ADJUSTED_QC');
            %netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], nitrate_adj_qc);
            ...
            netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], nitrate_adj);
            varid=netcdf.inqVarID(fid,'NITRATE_ADJUSTED_QC');
            netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], nitrate_adj_qc);


            nitrate_adj_error=MYBGC{FULLinds(5)-2}.NITRATE_ADJUSTED_ERROR;
            nitrate_adj_error(ii)=fillfloat; % qc='9'
            if(nidatamode=='D')
                nitrate_adj_error(kk)=fillfloat; % qc='4'
            end
            varid=netcdf.inqVarID(fid,'NITRATE_ADJUSTED_ERROR');
            netcdf.putVar(fid, varid, [0 nit_ax], [nbgcres(nit_ax-1,1) 1], nitrate_adj_error);
        end
    end
end
...
if FULLinds(4)>0 %ocr data exists
    temp_irr380=MYBGC{FULLinds(4)-2}.RAW_DOWNWELLING_IRRADIANCE380;
    if ~isempty(temp_irr380) %ie 0001 cycle 47....
        ...
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], temp_irr380);

        temp_irr380_qc=num2str(zeros(nbgcres(ocr_ax-1,1),1));
        %temp_irr380_qc(ii)='9';
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE380_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], temp_irr380_qc);

        temp_irr412=MYBGC{FULLinds(4)-2}.RAW_DOWNWELLING_IRRADIANCE412;
        %ii=find(temp_irr412>99998);
        %temp_irr412(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE412');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], temp_irr412);

        temp_irr412_qc=num2str(zeros(nbgcres(ocr_ax-1,1),1));
        %temp_irr412_qc(ii)='9';
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE412_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], temp_irr412_qc);

        temp_irr490=MYBGC{FULLinds(4)-2}.RAW_DOWNWELLING_IRRADIANCE490;
        %ii=find(temp_irr490>99998);
        %temp_irr490(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE490');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], temp_irr490);

        temp_irr490_qc=num2str(zeros(nbgcres(ocr_ax-1,1),1));
        %temp_irr490_qc(ii)='9';
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE490_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], temp_irr490_qc);

        temp_dwpar=MYBGC{FULLinds(4)-2}.RAW_DOWNWELLING_PAR;
        %ii=find(temp_dwpar>99998);
        %temp_dwpar(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_PAR');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], temp_dwpar);

        temp_dwpar_qc=num2str(zeros(nbgcres(ocr_ax-1,1),1));
        %temp_dwpar_qc(ii)='9';
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_PAR_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], temp_dwpar_qc);
        ...
        ocr380=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380;
        ii=find(ocr380>99998);
        ocr380(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE380');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr380);

        jj=find(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380_QC==99);
        if(length(jj)==nbgcres(ocr_ax-1,1) & length(ii)==nbgcres(ocr_ax-1,1))
            disp(strcat('no DOWN IRRADIANCE 380 in cast :', num2str(INFO.cast)));
            ocr380_qc=num2str(ones(nbgcres(ocr_ax-1,1),1).*9);
        else
            MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380_QC(ii)=9;
            ocr380_qc=num2str(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380_QC);
        end
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE380_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr380_qc);

        % write bgc adjusted data, error, and qc flags to nprof2 ------

        ocr380_adj=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380_ADJUSTED;
        ii=find(ocr380_adj>99998);

        jj=find(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380_ADJUSTED_QC==99);
        if(length(jj)==nbgcres(ocr_ax-1,1) & length(ii)==nbgcres(ocr_ax-1,1))
            disp(strcat('no DOWN_IRRADIANCE380_ADJUSTED in cast :', num2str(INFO.cast)));
        else
            MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380_ADJUSTED_QC(ii)=9;
            ocr380_adj_qc=num2str(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380_ADJUSTED_QC);
            %varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE380_ADJUSTED_QC');
            %netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr380_adj_qc);
            ...
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr380_adj);
    		varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE380_ADJUSTED_QC');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr380_adj_qc);

            ocr380_adj_error=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE380_ADJUSTED_ERROR;
            ...
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr380_adj_error);
        end
        % 2) SECOND BPARAM = DOWN_IRRADIANCE412---------------------------------------------------------------------------------------------------------------------------------------------
        ocr412=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412;
        ii=find(ocr412>99998);
        ocr412(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE412');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr412);

        jj=find(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412_QC==99);
        if(length(jj)==nbgcres(ocr_ax-1,1) & length(ii)==nbgcres(ocr_ax-1,1))
            disp(strcat('no DOWN IRRADIANCE 412 in cast :', num2str(INFO.cast)));
            ocr412_qc=num2str(ones(nbgcres(ocr_ax-1,1),1).*9);
        else
            MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412_QC(ii)=9;
            ocr412_qc=num2str(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412_QC);
        end
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE412_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr412_qc);

        % write bgc adjusted data, error, and qc flags to nprof2 ------

        ocr412_adj=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412_ADJUSTED;
        ii=find(ocr412_adj>99998);

        jj=find(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412_ADJUSTED_QC==99);
        if(length(jj)==nbgcres(ocr_ax-1,1) & length(ii)==nbgcres(ocr_ax-1,1))
            disp(strcat('no DOWN_IRRADIANCE412_ADJUSTED in cast :', num2str(INFO.cast)));
        else
            MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412_ADJUSTED_QC(ii)=9;
            ocr412_adj_qc=num2str(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412_ADJUSTED_QC);
            %varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE412_ADJUSTED_QC');
            %netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr412_adj_qc);
            ...
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr412_adj);
    		varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE412_ADJUSTED_QC');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr412_adj_qc);

            ocr412_adj_error=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE412_ADJUSTED_ERROR;
            ...
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr412_adj_error);
        end
        % 3) THIRD BPARAM = DOWN_IRRADIANCE490---------------------------------------------------------------------------------------------------------------------------------------------
        ocr490=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490;
        ii=find(ocr490>99998);
        ocr490(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE490');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr490);

        jj=find(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490_QC==99);
        if(length(jj)==nbgcres(ocr_ax-1,1) & length(ii)==nbgcres(ocr_ax-1,1))
            disp(strcat('no DOWN IRRADIANCE 490 in cast :', num2str(INFO.cast)));
            ocr490_qc=num2str(ones(nbgcres(ocr_ax-1,1),1).*9);
        else
            MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490_QC(ii)=9;
            ocr490_qc=num2str(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490_QC);
        end
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE490_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr490_qc);

        % write bgc adjusted data, error, and qc flags to nprof2 ------

        ocr490_adj=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490_ADJUSTED;
        ii=find(ocr490_adj>99998);

        jj=find(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490_ADJUSTED_QC==99);
        %     length(jj)
        %     length(ii)
        %     nbgcres4
        if(length(jj)==nbgcres(ocr_ax-1,1) & length(ii)==nbgcres(ocr_ax-1,1))
            ...
            MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490_ADJUSTED_QC(ii)=9;
            ocr490_adj_qc=num2str(MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490_ADJUSTED_QC);
            %varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE490_ADJUSTED_QC');
            %netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr490_adj_qc);
            ...
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr490_adj);
    		varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE490_ADJUSTED_QC');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr490_adj_qc);

            ocr490_adj_error=MYBGC{FULLinds(4)-2}.DOWN_IRRADIANCE490_ADJUSTED_ERROR;
            ...
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocr490_adj_error);
        end
        % 4) FOURTH BPARAM = DOWNWELLING_PAR---------------------------------------------------------------------------------------------------------------------------------------------
        ocrpar=MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR;
        ii=find(ocrpar>99998);
        ocrpar(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'DOWNWELLING_PAR');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocrpar);

        jj=find(MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR_QC==99);
        if(length(jj)==nbgcres(ocr_ax-1,1) & length(ii)==nbgcres(ocr_ax-1,1))
            disp(strcat('no DOWN IRRADIANCE 380 in cast :', num2str(INFO.cast)));
            ocrpar_qc=num2str(ones(nbgcres(ocr_ax-1,1),1).*9);
        else
            MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR_QC(ii)=9;
            ocrpar_qc=num2str(MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR_QC);
        end
        varid=netcdf.inqVarID(fid,'DOWNWELLING_PAR_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocrpar_qc);

        % write bgc adjusted data, error, and qc flags to nprof2 ------

        ocrpar_adj=MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR_ADJUSTED;
        ii=find(ocrpar_adj>99998);

        jj=find(MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR_ADJUSTED_QC==99);
        if(length(jj)==nbgcres(ocr_ax-1,1) & length(ii)==nbgcres(ocr_ax-1,1))
            disp(strcat('no DOWNWELLING_PAR_ADJUSTED in cast :', num2str(INFO.cast)));
        else
            MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR_ADJUSTED_QC(ii)=9;
            ocrpar_adj_qc=num2str(MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR_ADJUSTED_QC);
            %varid=netcdf.inqVarID(fid,'DOWNWELLING_PAR_ADJUSTED_QC');
            %netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocrpar_adj_qc);
            ...
            varid=netcdf.inqVarID(fid,'DOWNWELLING_PAR');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocrpar_adj);
    		varid=netcdf.inqVarID(fid,'DOWNWELLING_PAR_ADJUSTED_QC');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocrpar_adj_qc);

            ocrpar_adj_error=MYBGC{FULLinds(4)-2}.DOWNWELLING_PAR_ADJUSTED_ERROR;
            ...
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-1,1) 1], ocrpar_adj_error);
        end
    end
end

...
%
% APEX Iridium with Aanderaa 4330 (doxy), ISUS (nitrate), DURA (ph), FLBB (chla & bb)
%
% 5 B-Argo parameters: DOXY, CHLA, CHLA_FLUORESCENCE, BBP700, PH_IN_SITU_TOTAL, NITRATE
%
% 11 I-Argo parameters: TEMP_DOXY, TPHASE_DOXY, FLUORESCENCE_CHLA, BETA_BACKSCATTERING700, VRS_PH, IB_PH, IK_PH, VK_PH PH_IN_SITU_FREE, UV_INTENSITY_PARK_NITRATE, UV_INTENSITY_NITRATE (spectral)
%
% N_PARAM = 17 + 1 (PRES) = 18
%
%
% Annie Wong, January 2018
% Tanya Maurer, September 2021 added VK_PH and IK_PH, N_PARAM = 16+1(PRES) = 17
% Tanya Maurer, May 2024 added CHLA_FLUORESCENCE, = N_PARAM = 18
%--------------------------------------------------------------
...
station_params=...
[pres64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64, junk64;
pres64, tempdoxy64, tphasedoxy64, doxy64, fluorescence64, chla64, chlaF64, backscattering64, bbp64, vrsph64, ibph64, ikph64, vkph64, phfree64, phtotal64, uvdarknitrate64, uvnitrate64, nitrate64];
...
clear junk64 junkpres junktempdoxy junktphasedoxy junkdoxy junkfluorescence junkchla junkchlaF junkbackscattering junkbbp junkvrsph junkibph junkikph junkvkph junkphfree junkphtotal junkuvdarknitrate junkuvnitrate junknitrate


% assign DATA_MODE and PARAMETER_DATA_MODE ------

datamode_check=ones(1,6);
...
if(chlaFdatamode=='A')datamode_check(3)=2;end
if(chlaFdatamode=='D')datamode_check(3)=3;end

bbpdatamode=INFO.BBP700_DATA_MODE;
if(bbpdatamode=='A')datamode_check(4)=2;end
if(bbpdatamode=='D')datamode_check(4)=3;end

phdatamode=INFO.PH_DATA_MODE;
if(phdatamode=='A')datamode_check(5)=2;end
if(phdatamode=='D')datamode_check(5)=3;end

nidatamode=INFO.NITRATE_DATA_MODE;
if(nidatamode=='A')datamode_check(6)=2;end
if(nidatamode=='D')datamode_check(6)=3;end
...
param_datamode=...
['R', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ';
'R', 'R', 'R', doxydatamode, 'R', chladatamode, chlaFdatamode, 'R', bbpdatamode, 'R', 'R', 'R', 'R', 'R', phdatamode, 'R', 'R', nidatamode];
...
chlaF=LR.CHLA_FLUORESCENCE;
ii=find(chlaF>99998);
chlaF(ii)=fillfloat;
varid=netcdf.inqVarID(fid,'CHLA_FLUORESCENCE');
netcdf.putVar(fid, varid, [0 1], [nlowres 1], chlaF);

jj=find(LR.CHLA_FLUORESCENCE_QC==99);
if(length(jj)==nlowres & length(ii)==nlowres)
  disp(strcat('no CHLA_FLUORESCENCE in cast :', num2str(INFO.cast)));
  chlaF_qc=num2str(ones(nlowres,1).*9);
else
  LR.CHLA_FLUORESCENCE_QC(ii)=9;
  chlaF_qc=num2str(LR.CHLA_FLUORESCENCE_QC);
end
varid=netcdf.inqVarID(fid,'CHLA_FLUORESCENCE_QC');
netcdf.putVar(fid, varid, [0 1], [nlowres 1], chlaF_qc);
...
chlaF_adj=LR.CHLA_FLUORESCENCE_ADJUSTED;
ii=find(chlaF_adj>99998);

jj=find(LR.CHLA_FLUORESCENCE_ADJUSTED_QC==99);
if(length(jj)==nlowres & length(ii)==nlowres)
  disp(strcat('no CHLA_FLUORESCENCE_ADJUSTED in cast :', num2str(INFO.cast)));
else
  LR.CHLA_FLUORESCENCE_ADJUSTED_QC(ii)=9;
  chlaF_adj_qc=num2str(LR.CHLA_FLUORESCENCE_ADJUSTED_QC);
  varid=netcdf.inqVarID(fid,'CHLA_FLUORESCENCE_ADJUSTED_QC');
  netcdf.putVar(fid, varid, [0 1], [nlowres 1], chlaF_adj_qc);
  ...
  netcdf.putVar(fid, varid, [0 1], [nlowres 1], chlaF_adj);

  chlaF_adj_error=LR.CHLA_FLUORESCENCE_ADJUSTED_ERROR;
  ...
  netcdf.putVar(fid, varid, [0 1], [nlowres 1], chlaF_adj_error);
end
...
netcdf.putVar(fid, varid, [0,7-1,0,1], [256,1,1,1], chlaF_comment256); %chlaF=7
netcdf.putVar(fid, varid, [0,9-1,0,1], [256,1,1,1], bbp700_comment256); %bbp700=9
netcdf.putVar(fid, varid, [0,15-1,0,1], [256,1,1,1], ph_comment256); %ph=15
netcdf.putVar(fid, varid, [0,18-1,0,1], [256,1,1,1], ni_comment256); %nitrate=18
...
netcdf.putVar(fid, varid, [0,7-1,0,1], [256,1,1,1], chlaF_eqn256); %chlaF=7
netcdf.putVar(fid, varid, [0,9-1,0,1], [256,1,1,1], bbp700_eqn256); %bbp700=9
netcdf.putVar(fid, varid, [0,15-1,0,1], [256,1,1,1], ph_eqn256); %ph=15
netcdf.putVar(fid, varid, [0,18-1,0,1], [256,1,1,1], ni_eqn256); %nitrate=18
...
netcdf.putVar(fid, varid, [0,7-1,0,1], [256,1,1,1], chla_coeff256); %chla=7
netcdf.putVar(fid, varid, [0,9-1,0,1], [256,1,1,1], bbp700_coeff256); %bbp700=9
netcdf.putVar(fid, varid, [0,15-1,0,1], [256,1,1,1], ph_coeff256); %ph=15
netcdf.putVar(fid, varid, [0,18-1,0,1], [256,1,1,1], ni_coeff256); %nitrate=18

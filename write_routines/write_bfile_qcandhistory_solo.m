...
% %%% TM 02/16/23 NO SPIKE TEST!!!  remove...
% if FULLinds(3)>0 %eco data exists; FULLinds and MYBGC are defined in "write_mbaridata_solotype1"
%     chla=MYBGC{FULLinds(3)-2}.CHLA; %minus 2 for the 2 ctd axes
% 
...
%         % % %         if(isempty(missing)==1) && length(chla)>5 %only do this test if there is no missing value and if there are more than 5 values!
%         % % %
%         % % %             chla_runmedian=NaN.*ones(nbgcres(eco_ax-myindtmp+1,1));
%         % % %             for j=3:nbgcres(eco_ax-myindtmp+11,1)-2
%         % % %                 chla_runmedian(j)=median(chla(j-2:j+2));
%         % % %             end
%         % % %             chla_runmedian(1:2)=chla_runmedian(3);
%         % % %             chla_runmedian(nbgcres(eco_ax-myindtmp+1,1)-1:nbgcres(eco_ax-myindtmp+1,1))=chla_runmedian(nbgcres(eco_ax-myindtmp+1,1)-2);
...
%         % % %             index10=.1*nbgcres(eco_ax-myindtmp+11,1);

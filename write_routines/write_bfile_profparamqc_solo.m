...
if(exist('doxydatamode','var')==1)
    ...
    if(lennot9~=0)
        ...
        netcdf.putVar(fid, varid, [dox_ax], [1], profile_qcdoxy);
    end
end
...
if(exist('chladatamode','var')==1)
    ...
    if(lennot9~=0)
        ...
        netcdf.putVar(fid, varid, [eco_ax], [1], profile_qcchla);
    end
...
if(exist('chlaFdatamode','var')==1)
    ...
    if(lennot9~=0)
        ...
        netcdf.putVar(fid, varid, [eco_ax], [1], profile_qcchlaF);
    end
...
if(exist('bbpdatamode','var')==1)
    ...
    if(lennot9~=0)
        ...
        netcdf.putVar(fid, varid, [eco_ax], [1], profile_qcbbp);
    end
...
if(exist('cdomdatamode','var')==1)
    ...
    if(lennot9~=0)
        ...
        netcdf.putVar(fid, varid, [eco_ax], [1], profile_qccdom);
    end
...
if(exist('phdatamode','var')==1)
    ...
    if(lennot9~=0)
        ...
        netcdf.putVar(fid, varid, [ph_ax], [1], profile_qcph);
    end
...
if(exist('nidatamode','var')==1)
    ...
    if ~exist('nitrate_qc','var') %nothing in the file, so this variable was never created; fill values in Bfile matrix. ok.
        profile_qcni=' ';
        varid=netcdf.inqVarID(fid,'PROFILE_NITRATE_QC');
        netcdf.putVar(fid, varid, [nit_ax], [1], profile_qcni);
    else
...
if(exist('irr380datamode','var')==1)
    if ~exist('ocr380_qc','var') %ie cycle 47 0001; nothing in the file, so this variable was never created; fill values in Bfile matrix. ok.
        profile_qcocr380=' ';
        varid=netcdf.inqVarID(fid,'PROFILE_DOWN_IRRADIANCE380_QC');
        netcdf.putVar(fid, varid, [ocr_ax], [1], profile_qcocr380);
    else
...
if(exist('irr412datamode','var')==1)
    if ~exist('ocr412_qc','var') %ie cycle 47 0001; nothing in the file, so this variable was never created; fill values in Bfile matrix. ok.

        profile_qcocr412=' ';
        varid=netcdf.inqVarID(fid,'PROFILE_DOWN_IRRADIANCE412_QC');
        netcdf.putVar(fid, varid, [ocr_ax], [1], profile_qcocr412);
    else
...
if(exist('irr443datamode','var')==1)
    if ~exist('ocr443_qc','var') %ie cycle 47 0001; nothing in the file, so this variable was never created; fill values in Bfile matrix. ok.

        profile_qcocr443=' ';
        varid=netcdf.inqVarID(fid,'PROFILE_DOWN_IRRADIANCE443_QC');
        netcdf.putVar(fid, varid, [ocr_ax], [1], profile_qcocr443);
    else
...
if(exist('irr490datamode','var')==1)
    if ~exist('ocr490_qc','var') %ie cycle 47 0001; nothing in the file, so this variable was never created; fill values in Bfile matrix. ok.

        profile_qcocr490=' ';
        varid=netcdf.inqVarID(fid,'PROFILE_DOWN_IRRADIANCE490_QC');
        netcdf.putVar(fid, varid, [ocr_ax], [1], profile_qcocr490);
    else
...
% ***   OCR555   ***
if(exist('irr555datamode','var')==1)
    if ~exist('ocr555_qc','var') %ie cycle 47 0001; nothing in the file, so this variable was never created; fill values in Bfile matrix. ok.

        profile_qcocr555=' ';
        varid=netcdf.inqVarID(fid,'PROFILE_DOWN_IRRADIANCE555_QC');
        netcdf.putVar(fid, varid, [ocr_ax], [1], profile_qcocr555);
    else
        if(irr555datamode=='A'|irr555datamode=='D')
            qcocr555=ocr555_adj_qc;
        else
            qcocr555=ocr555_qc;
        end

        lenzero=length(find(qcocr555=='0')); %ocr only...as we begin, no QC!  All qc set to '0'=no qc performed.
        lennot9=length(find(qcocr555~='9'));
        lengood=length(find(qcocr555=='1'|qcocr555=='2'|qcocr555=='5'|qcocr555=='8'));
        if lenzero == length(qcocr555)
            profile_qcocr555 = ' ';
        else
            if(lennot9~=0)
                if lengood/lennot9 == 1
...
if(exist('pardatamode','var')==1)
    if ~exist('ocrpar_qc','var') %ie cycle 47 0001; nothing in the file, so this variable was never created; fill values in Bfile matrix. ok.

        profile_qcocrpar=' ';
        varid=netcdf.inqVarID(fid,'PROFILE_DOWNWELLING_PAR_QC');
        netcdf.putVar(fid, varid, [ocr_ax], [1], profile_qcocrpar);
    else

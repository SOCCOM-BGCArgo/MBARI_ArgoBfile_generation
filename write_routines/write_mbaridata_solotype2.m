...
% with N_PROF=7
%
% SOLO Rudix with SBE 83 (doxy), SUNA (nitrate), SBSpcomp (ph), ECO (chla & bb, cdom), OCR (irr380, irr443, irr490, irr555)
%
...
%
% ALL BGC SENSORS REPORT ON A SE555ATE PRESSURE AXIS SO N_PROF = 2 (CTD HR and LR) + 5 (BGC SENSORS) = 7
...
%
%
% Tanya Maurer, Feb 1, 2024
% 				TM update 7/2/24: addition of CHLA_FLUORESCENCE parameter.
%--------------------------------------------------------------
...
junktemp_rawdw555=char('RAW_DOWNWELLING_IRRADIANCE555',enough64');
rawd555_64=junktemp_rawdw555(1,:);
...
junkdw555=char('DOWN_IRRADIANCE555',enough64');
dw555_64=junkdw555(1,:);
...
OCRs = [rawdwirr380_64, rawdwirr443_64, rawdwirr490_64, rawd555_64, dwir380_64, dwir443_64, dwir490_64, dw555_64];
...
% % %     pres64, rawdwirr380_64, rawdwirr443_64, rawdwirr490_64, rawd555_64, dwir380_64, dwir443_64, dwir490_64, dw555_64;
...
clear TMP myrow station_params junk64 junkpres junktempdoxy junktphasedoxy junkdoxy junkfluorescence junkchla junkchlaF junkbackscattering junkbbp junkvrsph junkibph junkikph junkvkph junkphfree junkphtotal junkuvdarknitrate junkuvnitrate junknitrate junktemp_rawdwirr380 junktemp_rawdwirr443 junktemp_rawdwirr490 junktemp_rawdw555 junkdwirr380 junkdwirr443 junkdwirr490 junkdw555
...
for kk = 1:length(FULLinds)
    ...
    elseif FULLinds(kk)>0 && kk == 4 %OCR
        ...
        irr555datamode=INFO.DOWN_IRRADIANCE555_DATA_MODE;
        ...
        if(irr443datamode=='A')datamode_CHECK{kk}=[datamode_CHECK{kk} 2];end
        if(irr443datamode=='D')datamode_CHECK{kk}=[datamode_CHECK{kk} 3];end
        ...
        if(irr555datamode=='A')datamode_CHECK{kk}=[datamode_CHECK{kk} 2];end
        if(irr555datamode=='D')datamode_CHECK{kk}=[datamode_CHECK{kk} 3];end
...
% % %     'R', 'R', irr380datamode, 'R', irr443datamode, 'R', irr380datamode, 'R',555datamode;
% % %     'R', 'R', 'R', nidatamode, ' ', ' ', ' ', ' ', ' '];
...
OCRsDM = ['R', 'R', irr380datamode, 'R', irr443datamode, 'R', irr380datamode, 'R',irr555datamode];
...
if FULLinds(4)>0 %ocr data exists
    ...
    if ~isempty(temp_irr380) %ie 0001 cycle 47....
        ...
        temp_irr443=MYBGC{FULLinds(4)-myindtmp}.RAW_DOWNWELLING_IRRADIANCE443;
        %ii=find(temp_irr443>99998);
        %temp_irr443(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE443');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], temp_irr443);

        temp_irr443_qc=num2str(zeros(nbgcres(ocr_ax-myindtmp+1,1),1));
        %temp_irr443_qc(ii)='9';
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE443_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], temp_irr443_qc);
        ...
        temp_dw555=MYBGC{FULLinds(4)-myindtmp}.RAW_DOWNWELLING_IRRADIANCE555;
        %ii=find(temp_dw555>99998);
        %temp_dw555(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE555');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], temp_dw555);

        temp_dw555_qc=num2str(zeros(nbgcres(ocr_ax-myindtmp+1,1),1));
        %temp_dw555_qc(ii)='9';
        varid=netcdf.inqVarID(fid,'RAW_DOWNWELLING_IRRADIANCE555_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], temp_dw555_qc);
        ...
        ocr443=MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443;
        ii=find(ocr443>99998);
        ocr443(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE443');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr443);

        jj=find(MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443_QC==99);
        if(length(jj)==nbgcres(ocr_ax-myindtmp+1,1) & length(ii)==nbgcres(ocr_ax-myindtmp+1,1))
            disp(strcat('no DOWN IRRADIANCE 443 in cast :', num2str(INFO.cast)));
            ocr443_qc=num2str(ones(nbgcres(ocr_ax-myindtmp+1,1),1).*9);
        else
            MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443_QC(ii)=9;
            ocr443_qc=num2str(MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443_QC);
        end
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE443_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr443_qc);

        % write bgc adjusted data, error, and qc flags to nprof2 ------

        ocr443_adj=MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443_ADJUSTED;
        ii=find(ocr443_adj>99998);

        jj=find(MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443_ADJUSTED_QC==99);
        if(length(jj)==nbgcres(ocr_ax-myindtmp+1,1) & length(ii)==nbgcres(ocr_ax-myindtmp+1,1))
            disp(strcat('no DOWN_IRRADIANCE443_ADJUSTED in cast :', num2str(INFO.cast)));
        else
            MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443_ADJUSTED_QC(ii)=9;
            ocr443_adj_qc=num2str(MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443_ADJUSTED_QC);
            %varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE443_ADJUSTED_QC');
            %netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr443_adj_qc);

            kk=find(ocr443_adj_qc=='4');
            ocr443_adj(ii)=fillfloat; % qc='9'
            if(irr443datamode=='D')
                ocr443_adj(kk)=fillfloat; % qc='4'
                % 			ocr443_adj_qc(kk)='9'; % qc='4'
            end
            varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE443_ADJUSTED');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr443_adj);
    		varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE443_ADJUSTED_QC');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr443_adj_qc);

            ocr443_adj_error=MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE443_ADJUSTED_ERROR;
            ...
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr443_adj_error);
        end
        ...
        % 4) FOURTH Bparam = DOWN_IRRADIANCE555---------------------------------------------------------------------------------------------------------------------------------------------
        ocr555=MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555;
        ii=find(ocr555>99998);
        ocr555(ii)=fillfloat;
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE555');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr555);

        jj=find(MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555_QC==99);
        if(length(jj)==nbgcres(ocr_ax-myindtmp+1,1) & length(ii)==nbgcres(ocr_ax-myindtmp+1,1))
            disp(strcat('no DOWN IRRADIANCE 555 in cast :', num2str(INFO.cast)));
            ocr555_qc=num2str(ones(nbgcres(ocr_ax-myindtmp+1,1),1).*9);
        else
            MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555_QC(ii)=9;
            ocr555_qc=num2str(MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555_QC);
        end
        varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE555_QC');
        netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr555_qc);

        % write bgc adjusted data, error, and qc flags to nprof2 ------

        ocr555_adj=MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555_ADJUSTED;
        ii=find(ocr555_adj>99998);

        jj=find(MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555_ADJUSTED_QC==99);
        if(length(jj)==nbgcres(ocr_ax-myindtmp+1,1) & length(ii)==nbgcres(ocr_ax-myindtmp+1,1))
            disp(strcat('no DOWN_IRRADIANCE555_ADJUSTED in cast :', num2str(INFO.cast)));
        else
            MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555_ADJUSTED_QC(ii)=9;
            ocr555_adj_qc=num2str(MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555_ADJUSTED_QC);
            %varid=netcdf.inqVarID(fid,'DOWNWELLING_555_ADJUSTED_QC');
            %netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr555_adj_qc);

            kk=find(ocr555_adj_qc=='4');
            ocr555_adj(ii)=fillfloat; % qc='9'
            if(irr555datamode=='D')
                ocr555_adj(kk)=fillfloat; % qc='4'
                % 			ocr555_adj_qc(kk)='9'; % qc='4'
            end
            varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE555');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr555_adj);
    		varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE555_ADJUSTED_QC');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr555_adj_qc);

            ocr555_adj_error=MYBGC{FULLinds(4)-myindtmp}.DOWN_IRRADIANCE555_ADJUSTED_ERROR;
            ocr555_adj_error(ii)=fillfloat; % qc='9'
            if(irr555datamode=='D')
                ocr555_adj_error(kk)=fillfloat; % qc='4'
            end
            varid=netcdf.inqVarID(fid,'DOWN_IRRADIANCE555_ADJUSTED_ERROR');
            netcdf.putVar(fid, varid, [0 ocr_ax], [nbgcres(ocr_ax-myindtmp+1,1) 1], ocr555_adj_error);
        end
    end
end
...
if FULLinds(4)>0 %ocr data exists
    ...
    junk=char(INFO.DOWN_IRRADIANCE555_SCI_CAL_EQU,enough256');
    ocr555_eqn256=junk(1,:);
    junk=char(INFO.DOWN_IRRADIANCE555_SCI_CAL_COEF,enough256');
    ocr555_coeff256=junk(1,:);
    junk=char(INFO.DOWN_IRRADIANCE555_SCI_CAL_COM,enough256');
    ocr555_comment256=junk(1,:);

    netcdf.putVar(fid, varid_sccom, [0,3-1,0,FULLinds(4)-1], [256,1,1,1], ocr380_comment256); %ocr380
    netcdf.putVar(fid, varid_sccom, [0,5-1,0,FULLinds(4)-1], [256,1,1,1], ocr443_comment256); %ocr443
    netcdf.putVar(fid, varid_sccom, [0,7-1,0,FULLinds(4)-1], [256,1,1,1], ocr490_comment256); %ocr490
    netcdf.putVar(fid, varid_sccom, [0,9-1,0,FULLinds(4)-1], [256,1,1,1], ocr555_comment256); %ocr555
    netcdf.putVar(fid, varid_sceq, [0,3-1,0,FULLinds(4)-1], [256,1,1,1], ocr380_eqn256); %ocr380
    netcdf.putVar(fid, varid_sceq, [0,5-1,0,FULLinds(4)-1], [256,1,1,1], ocr443_eqn256); %ocr443
    netcdf.putVar(fid, varid_sceq, [0,7-1,0,FULLinds(4)-1], [256,1,1,1], ocr490_eqn256); %ocr490
    netcdf.putVar(fid, varid_sceq, [0,9-1,0,FULLinds(4)-1], [256,1,1,1], ocr555_eqn256); %ocr555
    netcdf.putVar(fid, varid_sccof, [0,3-1,0,FULLinds(4)-1], [256,1,1,1], ocr380_coeff256); %ocr380
    netcdf.putVar(fid, varid_sccof, [0,5-1,0,FULLinds(4)-1], [256,1,1,1], ocr443_coeff256); %ocr443
    netcdf.putVar(fid, varid_sccof, [0,7-1,0,FULLinds(4)-1], [256,1,1,1], ocr490_coeff256); %ocr490
    netcdf.putVar(fid, varid_sccof, [0,9-1,0,FULLinds(4)-1], [256,1,1,1], ocr555_coeff256); %ocr555
    for i=2:9
